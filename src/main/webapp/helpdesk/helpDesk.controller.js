sap.ui.controller("helpdesk.helpDesk", {
    ...
    /**
     * calls user data servlet for user information. stores user data (name, email, app name) in the global user model
    ...
    /**
     * Initialization of the gamification service notification widget at some point the notification configuration will
    ...
    /**
     * loads helpdesk tickets from server resets local notifications. this method is called: - on application start - on
     * logout (remark: no real logout. only jumps back to welcome screen) - to reset tickets after all available tickets
    ...
    initializeData: function (callback, scope) {
        ...
        if (!sap.ui.getCore().getModel("notifications")) {
            sap.ui.getCore().setModel(new sap.ui.model.json.JSONModel(), "notifications");
        }
