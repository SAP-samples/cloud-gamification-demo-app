...
var GSNotifications = (function () {
    ...
    function _init(config) {
        ...
        _log("Notification module initialized.");
    ...
    function _groupPoints(bState) {
        if (typeof bState === "boolean") {
            ...
            _log(`Grouping point notifications is now ${((bState) ? "enabled." : "disabled.")}`);
        } else {
    ...
    function _setAppName(sAppName) {
        if (typeof sAppName === "string" && sAppName !== "") {
            ...
            _log(`Please provide a valid string. App context continues to be ${(_config.appName === undefined || _config.appName === null) ? "default app" : _config.appName}`);
        }
    }
    ...
    function _getServerTime() {
        ...
        _sendAPIRequest(request, function (data) {
            if (data.result !== null) {
                ...
                _log(`Current server time: ${new Date(data.result)}`);
            }
        });
    }
    ...
    function _showPoints(bState) {
        if (typeof bState === "boolean") {
            ...
            _log(`Notifications for earned points are now ${((bState) ? "enabled." : "disabled.")}`);
        } else {
            _log(`ERROR. ${bState} must be of type boolean.`);
        }
    }
    ...
    return {
        ...
        setStyleClasses: _setStyleClasses, // list all style classes but dont forget to
